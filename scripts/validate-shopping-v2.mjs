...
const EXPECTED_SANITIZED_INDEX_SHA256 = "34b1fd6e47caad8a3c8e452c8fc9981d41b4468cab41f4f268e6a0987178fb74";
...
await check("Meal portion UI keeps contextual and generic controls separate", () => {
  ...
  vm.runInNewContext(
    ["normalizePeopleCount", "recipeReadingMetadata", "recipeEssentialHTML", "recipeIngredientsHTML"].map(name=>extractFunction(indexSource,name)).join("\n") + "\n" +
      indexSource.slice(indexSource.indexOf('function escapeHTML('),indexSource.indexOf('let V73_NOTES_RAW')) + "\n" +
