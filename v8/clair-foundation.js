(() => {
  ...
  async function verifyCurrentSnapshot(record, kind) {
    if (
      !compatibleSnapshot(record) ||
      typeof kind !== 'string' ||
      !kind ||
      record.id !== `${APP_ID}:${kind}` ||
      record.kind !== kind ||
    ...
    const persisted = await getSnapshot(kind);
    return Boolean(
      compatibleSnapshot(persisted) &&
      persisted.id === record.id &&
      persisted.kind === kind &&
      persisted.release === RELEASE &&
      persisted.coreRevision === CORE_REVISION &&
      persisted.capturedAt === record.capturedAt &&
  ...
  window.ClairV8 = Object.freeze({
    ...
    async snapshot(kind = 'manual') {
      const written = await putSnapshot(kind);
      if (!written) return null;
      return (await verifyCurrentSnapshot(written, kind)) ? written : null;
    },
    async verifySnapshot(record, kind) {
      return verifyCurrentSnapshot(record, kind);
    },
    async restoreLatest() {
